...
async function notify(db, userId, title, message, link) {
  await db.execute(`INSERT INTO notifications (id,user_id,title,message,type,is_read,link)
    VALUES (?,?,?,?, 'payment',FALSE,?)`, [crypto.randomUUID(), userId, title, message, link]);
}
...
export async function paymentContext(req, res, next) {
  try {
    const [rows] = await pool.execute(`SELECT o.*,u.bkash_number,u.nagad_number,u.rocket_number,u.upay_number,
      ...
      JOIN users u ON u.id=o.seller_id WHERE o.id=? AND (o.buyer_id=? OR ?='admin') LIMIT 1`,
...
export async function createTransaction(req, res, next) {
  ...
  try {
    ...
    const [orders] = await db.execute(`SELECT o.*,u.bkash_number,u.nagad_number,u.rocket_number,u.upay_number,
      ...
      WHERE o.id=? AND o.buyer_id=? FOR UPDATE`, [body.order_id, req.user.id]);
    ...
    const [created] = await db.execute(`${detailSql} WHERE t.id=?`, [id]);
    await db.commit();
...
async function listFor(req, res, next, field) {
  try {
    ...
    const [rows] = await pool.execute(`${detailSql} WHERE t.${field}=? ORDER BY t.created_at DESC LIMIT ${limit} OFFSET ${offset}`, [req.user.id]);
    const [[count]] = await pool.execute(`SELECT COUNT(*) total FROM transactions WHERE ${field}=?`, [req.user.id]);
...
export async function getTransaction(req, res, next) {
  try {
    const [rows] = await pool.execute(`${detailSql} WHERE t.id=? LIMIT 1`, [req.params.id]);
    const row = rows[0];
...
export async function updateStatus(req, res, next) {
  ...
  try {
    await db.beginTransaction();
    const [rows] = await db.execute('SELECT * FROM transactions WHERE id=? FOR UPDATE', [req.params.id]);
    const tx = rows[0];
    ...
    await db.execute('UPDATE transactions SET status=? WHERE id=?', [status, tx.id]);
    if (['received', 'verified'].includes(status)) await db.execute("UPDATE orders SET payment_status='paid' WHERE id=?", [tx.order_id]);
    ...
    const [updated] = await db.execute(`${detailSql} WHERE t.id=?`, [tx.id]);
    await db.commit();
...
export async function adminList(req, res, next) {
  try {
    ...
    ['status', 'payment_method', 'buyer_id', 'seller_id'].forEach((field) => {
      if (req.query[field]) { clauses.push(`t.${field}=?`); values.push(req.query[field]); }
    });
    if (req.query.date_from) { clauses.push('DATE(t.created_at)>=?'); values.push(req.query.date_from); }
    if (req.query.date_to) { clauses.push('DATE(t.created_at)<=?'); values.push(req.query.date_to); }
    if (req.query.search) {
      clauses.push('(t.transaction_code LIKE ? OR t.transaction_reference LIKE ? OR o.buyer_name LIKE ? OR o.seller_name LIKE ?)');
      const term = `%${req.query.search}%`; values.push(term, term, term, term);
    }
  ...
  try {
    const [[totals], [methods], [months]] = await Promise.all([
      pool.execute(`SELECT COUNT(*) total_transactions,
        ...
        SUM(payment_method='cash_on_delivery') cod_transactions FROM transactions`),
      pool.execute('SELECT payment_method name,COUNT(*) value FROM transactions GROUP BY payment_method ORDER BY value DESC'),
      pool.execute(`SELECT DATE_FORMAT(created_at,'%Y-%m') month,SUM(amount) amount,COUNT(*) transactions
