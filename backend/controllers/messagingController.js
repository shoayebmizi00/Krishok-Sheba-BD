...
function parseParticipants(row) {
  if (Array.isArray(row?.participant_ids) && row.participant_ids.length > 0) return row.participant_ids;
  try {
    const parsed = JSON.parse(row?.participant_ids || '[]');
    if (Array.isArray(parsed) && parsed.length > 0) return parsed;
  } catch {
    // Fall back to normalized participant columns.
  }
...
export async function createConversation(req, res, next) {
  try {
    ...
    const relatedType = req.body.related_type || (req.body.listing_id ? 'listing' : 'general');
    ...
    const relation = ['general', 'user'].includes(relatedType) ? null : await relationFor(relatedType, relatedId);

    if (!canStartRelatedConversation({ user: currentUser, receiver, relatedType, relation })) {
  ...
  try {
    ...
    res.json({
      ...
      messages: messages.map((item) => ({
        ...item,
        message_text: item.message_text || item.content,
...
export async function sendMessage(req, res, next) {
  ...
  try {
    ...
    const conversation = await findConversation(req.body.conversation_id, db);
    if (!conversation || !isConversationParticipant(conversation, req.user.id, req.user.role)) {
      await db.rollback();
    ...
    await db.execute(
      ...
      [
        ...
        receiverId,
        'নতুন বার্তা পেয়েছেন',
        `${senderName} আপনাকে একটি নতুন বার্তা পাঠিয়েছেন।`,
    ...
    res.status(201).json({
      ...created[0],
      message_text: created[0].message_text || created[0].content,
      created_date: created[0].created_at
    });
  } catch (error) {
    await db.rollback();
    next(error);
